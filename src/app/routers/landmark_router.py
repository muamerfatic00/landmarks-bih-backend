...
async def create_landmark(data: LandmarkPostRequest,
                          landmark_service: BaseService = Depends(get_base_service(Landmark))) -> LandmarkBaseResponse:
    return await landmark_service.create(data, LandmarkBaseResponse)
...
async def get_paginated_landmarks(page: int = Query(default=1), page_size: int = Query(default=10),
                                  landmark_service: BaseService = Depends(get_base_service(Landmark))) -> Page[
    LandmarkDetailResponse]:
    try:
        pagination = PaginationParam(page=page, page_size=page_size)
        return await landmark_service.get_paginated(pagination, LandmarkDetailResponse)
...
async def get_landmark_by_id(_id: int,
                             landmark_service: BaseService = Depends(
                                 get_base_service(Landmark))) -> LandmarkDetailResponse:
    return await landmark_service.get_by_id(_id, LandmarkDetailResponse)


@landmark_router.get("", response_model=list[LandmarkDetailResponse], status_code=status.HTTP_200_OK)
async def get_all_landmarks(landmark_service: BaseService = Depends(get_base_service(Landmark))) -> [
    list[LandmarkDetailResponse]]:
    return await landmark_service.get_all(LandmarkDetailResponse)


@landmark_router.put("/{_id}", response_model=LandmarkBaseResponse, status_code=status.HTTP_200_OK)
async def update_landmark(_id: int, data_for_update: LandmarkPutRequest,
                          landmark_service: BaseService = Depends(get_base_service(Landmark))) -> LandmarkBaseResponse:
    ...
    return await landmark_service.update(_id, data_for_update, LandmarkBaseResponse)


@landmark_router.delete("/{_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_landmark(_id: int, landmark_service: BaseService = Depends(get_base_service(Landmark))) -> None:
    await landmark_service.delete_by_id(_id)
